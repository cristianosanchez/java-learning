...
public class Counter extends DefaultHandler {
	...
	public void printResults(PrintWriter out, String uri, long time,
			long memory, boolean tagginess, int repetition) {
	...
	protected void printError(String type, SAXParseException ex) {
		...
		output.print(':');
		output.print(ex.getLineNumber());
		output.print(':');
		output.print(ex.getColumnNumber());
		output.print(": ");
	...
	public static void main(String argv[]) {
		...
		for (int i = 0; i < argv.length; i++) {
			...
			if (arg.startsWith("-")) {
				...
				if (option.equals("p")) {
					...
					try {
						...
						try {
							Parser sax1Parser = ParserFactory
									.makeParser(parserName);
							parser = new ParserAdapter(sax1Parser);
							System.err
									.println("warning: Features and properties not supported on SAX1 parsers.");
						} catch (Exception ex) {
							parser = null;
							System.err
			...
			try {
				...
				System.err
						.println("warning: Parser does not recognize feature ("
			...
			try {
				...
				if (filePath.isFile()) {
					...
					for (int j = 0; j < repetition; j++) {
						try {
							System.err.println("*** parsing " + filePath);
							parser.parse(filePath.getAbsolutePath());
						} catch(SAXException e) {
					...
					if (fileList != null && fileList.length > 0) {
						File logFile = new File(filePath, "xml.log");
						output = new PrintWriter(logFile);
						
						for (String xmlFilePath : fileList) {
							File xmlFile = new File(arg, xmlFilePath);
							for (int j = 0; j < repetition; j++) {
								try {
									System.out.println("parsing file " + xmlFile);
									output.println("*** ARQUIVO:" + xmlFile);
